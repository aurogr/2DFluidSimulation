...
// Changes to Fluid.h to add the apicEnabled variable for APIC
...
public:
    Fluid2(const Grid2 &grid)
        : grid(grid)
        , flipEnabled(true)
        , apicEnabled(false) // in the code apic is an upgrade from flip, so apic can't be enabled if flip isn't
    {
    }
